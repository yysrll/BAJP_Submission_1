...
class LocalRepository(context: Context) {
    ...
    fun insertFavoriteMovie(movie: MovieEntity) =
        executorService.execute { movieDao.insertFavoriteMovie(movie) }
    ...
    fun getFavoriteTvShowById(id: Int): LiveData<List<TvShowEntity>> =
        tvShowDao.getFavoriteTvShowById(id)

    fun insertFavoriteTvShow(tvShow: TvShowEntity) =
        executorService.execute { tvShowDao.insertFavoriteTvShow(tvShow) }

    fun deleteFavoriteTvShow(id: Int) =
        executorService.execute { tvShowDao.deleteFavoriteTvShow(id) }
